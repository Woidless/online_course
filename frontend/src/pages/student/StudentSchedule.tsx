...
export default function StudentSchedule() {
  ...
  useEffect(() => {
    api.get<Schedule[]>('/schedule/')
      .then(res => { setSchedule(res.data); setError(null) })
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-semibold text-gray-900 dark:text-gray-100">Расписание</h1>

      {schedule.length === 0 ? (
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-12 text-center">
          <p className="text-gray-500 dark:text-gray-400">Нет запланированных занятий</p>
        </div>
      ) : (
          ...
          {schedule.map(item => (
            <div key={item.id} className="flex items-center justify-between p-5 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
              <div className="flex-1">
                ...
                {item.course_title && (
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{item.course_title}</p>
                )}
              </div>
